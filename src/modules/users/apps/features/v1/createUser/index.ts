...
import { DataResponse as ApiDataResponse, DataResponseFactory } from "@/shared/models/response/data.Response";
...
import { CreateUserRequestDto, CreateUserResponseDto } from "../../../contracts/v1/createUser/index.Contract";
...
export class CreateUserController{
  ...
  public async handleAsync(@Body() request: AesRequestDto,@Res() res:Response){
    const response=await medaitR.send<ApiDataResponse<AesResponseDto>>(new CreateUserCommand(request));
...
export class CreateUserCommandHandler implements RequestHandler<CreateUserCommand,ApiDataResponse<AesResponseDto>>{
  ...
  public constructor(){
    ...
    this._createUserValidationRequestService=Container.get(CreateUserValidationRequestService);
  ...
  public async handle(value: CreateUserCommand): Promise<ApiDataResponse<AesResponseDto>> {
   try{
    if(!value)
      return DataResponseFactory.error<AesResponseDto>(StatusCodes.BAD_REQUEST,'Value parameter is required');

    if(!value.request)
      return DataResponseFactory.error<AesResponseDto>(StatusCodes.BAD_REQUEST,'Request Object is required');

    const aesRequestDto:AesRequestDto=value.request;

    // Decrypt Request Service
    const decryptRequestServiceResult=await this._createUserDecryptRequestService.handleAsync({
      data:aesRequestDto.body,
      key:ENCRYPTION_KEY
    });
    if(decryptRequestServiceResult.isErr())
      return DataResponseFactory.error(decryptRequestServiceResult.error.httpCode,decryptRequestServiceResult.error.message);

    const createUserRequestDto:CreateUserRequestDto=decryptRequestServiceResult.value;

    // Validation Request Service
    const validationRequestServiceResult=await this._createUserValidationRequestService.handleAsync({
      dto:createUserRequestDto,
      dtoClass:CreateUserRequestDto
    });
    if(validationRequestServiceResult.isErr())
      return DataResponseFactory.error(validationRequestServiceResult.error.httpCode,validationRequestServiceResult.error.message);
    ...
    if(createUserDbServiceResult.isErr())
      return DataResponseFactory.error(createUserDbServiceResult.error.httpCode,createUserDbServiceResult.error.message);

    const createUserDdbResultResponse:ICreateUserDbServiceResults=createUserDbServiceResult.value;

    // UserCreated Event
    await medaitR.publish(new UserCreatedDomainEvent(createUserRequestDto.email,createUserRequestDto.fullName));
    ...
    const createUserEncryptResponseResult=await this._createUserEncryptResponseService.handleAsync({
      data:response,
      key:ENCRYPTION_KEY
    });
    if(createUserEncryptResponseResult.isErr())
      return DataResponseFactory.error(createUserEncryptResponseResult.error.httpCode,createUserEncryptResponseResult.error.message);

    const aesResponseDto:AesResponseDto=createUserEncryptResponseResult.value.aesResponseDto;

    // Success
    return DataResponseFactory.success(StatusCodes.CREATED,aesRequestDto,`User ${createUserRequestDto.email} created successfully`);

   }
   catch(ex){
    const error=ex as Error;
    return DataResponseFactory.error<AesResponseDto>(StatusCodes.INTERNAL_SERVER_ERROR,error.message);
   }
  }
